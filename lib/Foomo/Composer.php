...
class Composer
{
	public static function generatePackages()
	{
		...
		foreach(\Foomo\Modules\Manager::getEnabledModules() as $enabledModule) {
			...
			if(!file_exists($filename) && !file_exists($overrideFilename)) {
				continue;
			} else if(file_exists($filename)) {
				$fileSize = filesize($filename);
				if($fileSize > 0) {
					// if not autogenerated => skip
					$existingPackage = json_decode(file_get_contents($filename));
					if(!(isset($existingPackage->extra) && isset($existingPackage->extra->foomoAutoGenerated) && $existingPackage->extra->foomoAutoGenerated === true)) {
						continue;
					}
				}
			}
